...
texture_air_icon,texture_bullet_lf,texture_bullet_rg,texture_bullet_icon, texture_drug, texture_winner,
texture_stun, texture_number[10];


sf::SoundBuffer jump_buffer,explo_nuke_buffer,explo_tnt_buffer,stuka_buffer,bullet_buffer,power_buffer;
sf::Sound jumpSound,explo_nukeSound,explo_tntSound,stukaSound,bulletSound,powerSound;
const ld speed = 4;
ld player_lf_speed = speed,  player_rg_speed = speed;
struct object{
    sf::Sprite sprite;
    bool isDraw = 1,is_jump = 0,is_fall = 0,is_picked = 0,is_throw = 0,character = 0,jumping = 0, dizzy = 0;;
    ld t_jump = 0,t_fall = 0,t = 0,direct = 0, odirect = 0,onDrug = 0, cloud_speed = 0.03;
    object *carry;
    ...
    void cloud_move()
    {
        sf::Vector2f pos = sprite.getPosition();
        pos.x += cloud_speed;
        if((cloud_speed < 0 && pos.x < 0) || (cloud_speed > 0 && pos.x > 1120))cloud_speed *= -1;
...
void Change_Gamestate(){
    ...
    if(!drug.isDraw)
    {
        if(timerdrug.getElapsedTime().asSeconds() > 12.f)
        {
            drug.isDraw = 1;
            timerdrug.restart();
    ...
    else
    {
        ...
        else if(timerdrug.getElapsedTime().asSeconds() > 10.f)
        {
            drug.isDraw = 0;
            timerdrug.restart();
    ...
    {
        if(player_lf_speed > speed)player_lf_speed = speed;
        if(player_rg_speed > speed)player_rg_speed = speed;
